...
import { ArrowLeft, Target, Flame, Clock, Plus, User, Copy, ArrowRightCircle } from "lucide-react";
...
function PlanifierContent() {
  ...
  const [actionsList, setActionsList] = useState<ActionInput[]>([
    { content: searchParams.get("action") || "", isMust: false, minutes: 15, owner: "Moi" },
  ]);
  ...
  function addActionRow() {
    setActionsList((prev) => [...prev, { content: "", isMust: false, minutes: 15, owner: "Moi" }]);
  }
  ...
  async function handleCreateBlock(e: React.FormEvent) {
    ...
    if (res.ok) {
      setResult("");
      setPurpose("");
      setActionsList([{ content: "", isMust: false, minutes: 15, owner: "Moi" }]);
      loadData();
    }
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 p-4 sm:p-6">
      ...
      <form onSubmit={handleCreateBlock} className="space-y-5 rounded-2xl border border-white/10 bg-[#0d0d10] p-6 shadow-xl">
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="Ex: Doubler mes ventes du trimestre"
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full rounded-xl border border-white/10 bg-black/60 px-3 py-2 text-xs text-zinc-200"
          >
        ...
        <div className="space-y-3 border-t border-white/10 pt-3">
          <label className="text-xs font-bold text-amber-300 uppercase block">3. Plan d&apos;Actions Massives (M)</label>
          ...
          {actionsList.map((act, index) => (
            <div key={index} className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-black/60 px-2.5 py-2 text-xs text-zinc-300">
                ...
                <input
                  value={act.owner}
                  onChange={(e) => updateActionRow(index, { owner: e.target.value })}
      ...
      {/* Liste des blocs avec duplication et suivi */}
      <div className="space-y-4">
        <h2 className="text-sm font-bold text-zinc-300 uppercase">Blocs RPM ({blocks.length})</h2>
        {blocks.map((b) => (
          <div key={b.id} className="rounded-2xl border border-white/10 bg-[#0d0d10] p-5 space-y-3">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleBlockAction(b.id, "duplicate")}
                  className="flex items-center gap-1 rounded-lg border border-white/10 bg-black/40 px-2.5 py-1 text-xs font-semibold text-zinc-300 hover:text-amber-300"
                  title="Dupliquer le bloc"
                >
                  <Copy className="h-3.5 w-3.5" /> Dupliquer
                </button>
                <button
                  onClick={() => handleBlockAction(b.id, "carryOver")}
                  ...
                  title="Mettre en suivi pour la semaine prochaine"
                >
                  <ArrowRightCircle className="h-3.5 w-3.5" /> Reporter Semaine +1
                </button>
              </div>
            </div>
